...
export function createRealtimeAppServerRoutes(options: {
      ...
      try {
        ...
        answer = await session.start({
          ...
          timezone: payload.session?.timezone
            ...
            ?? "UTC",
          timezoneOffsetMinutes: payload.session?.timezoneOffsetMinutes
            ?? serverNow.getTimezoneOffset(),
        });
      } catch {
  ...
  async function refreshClock(request: Request, liveSessionId: string): Promise<Response> {
    const ownerSessionId = await options.readSessionId(request);
    if (!ownerSessionId) return json({ error: "not_authenticated" }, { status: 401 });
    const managed = getSession(liveSessionId, ownerSessionId);
    if (!managed) return json({ error: "realtime_session_not_found" }, { status: 404 });
    if (!managed.session.refreshClock) {
      return json({ error: "realtime_clock_refresh_not_supported" }, { status: 501 });
    }
    try {
      await managed.session.refreshClock();
      return json({ status: "refreshed" });
    } catch {
      return json({ error: "realtime_clock_refresh_failed" }, { status: 500 });
    }
  }
